...
void terminal_clear_line(void) {
  size_t start = terminal_row * VGA_WIDTH;
  size_t limit = start + VGA_WIDTH;
...
void terminal_left_column(void) {
...
void terminal_right_column(void) {
  size_t next_index = terminal_row * VGA_WIDTH + terminal_column + 1;

  if (terminal_column < VGA_WIDTH - 1) {
    if ((terminal_buffer[next_index] & 0xFF) != ' ') {
      terminal_column++;
      terminal_update_cursor(terminal_column, terminal_row);
    }
  }
